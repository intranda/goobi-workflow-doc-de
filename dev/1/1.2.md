...
### Git-Eigenschaften
Ausserdem sollten für die Nutzung von Git die folgenden beiden Eigenschaften `email` und `user` korrekt konfiguriert sein. Dies kann innerhalb der Eclipse Einstellungen in dem Bereich `Version Control (Team)` - `Git` - `Configuration` erfolgen. Dort sollte die beiden Parameter `user.namej` und `user.email` gesetzt werden.
