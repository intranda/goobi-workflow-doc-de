...
Durch ein Update des Plugins muss ein `type=""` für jede \<rule\> gesetzt werden. Der Typ von bestehenden Regeln ist immer `filter`.
Dafür muss in der Config-Datei plugin\_intranda\_administration\_data\_poller.xml in jedem \<rule\>-Tag `type="filter"` gesetzt werden.
