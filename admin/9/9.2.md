...
```bash
mysql -NBse 'select replace(NAME,"/",".") from information_schema.innodb_sys_tables where name regexp "^goobi/" and row_format="Compact"' | xargs -I€ mysql goobi -NBse 'alter table € row_format=Dynamic'
```
