...
| `MetsEditorImageSize` | Text | | Mit dieser Einstellung kann eine Liste von unterstützten Bilddateigrößen angegeben werden, die später bei einer API-Anfrage via "/info.json" oder "/{process}/{folder}/{filename}/info.json" zurückgegeben werden. |
<!--- hier Listenoptionen angeben -->
| `MetsEditorImageTileSize` | Text | | LIST |
<!--- hier Listenoptionen angeben -->
| `MetsEditorImageTileScale` | Text | | LIST |
<!--- hier Listenoptionen angeben -->
...
| `MaxImageFileSize` | Number | `4000` | Dieser Wert gibt an, wie groß Bilddateien maximal sein dürfen, um im Metadateneditor verwendet werden zu können. Wichtig dafür ist auch die korrekte Anwendung der Einstellung `MaxImageFileSizeUnit`, mit der ein Faktor für die hier angegebene Zahl konfiguriert wird. |
| `MaxImageFileSizeUnit` | Text | `MB` | Dieser Wert kann wie folgt gesetzt werden: |
<!--- Hier alle Werte auflisten -->
...
| `historyImageSuffix` | Text | `.tif` | Beim Abschließen von Schritten werden verschiedene interne Datenüberprüfungen vorgenommen. Unter anderem wird die Anzahl an vorhandenen und bearbeiteten Bilddateien überprüft. Mit diesem Wert kann eine Liste von Dateitypen angegeben werden, die dabei gezählt und verglichen werden. |
<!--- hier Listenoptionen angeben -->
...
| `ExportMetadataForProject` | Text | | Dieser Wert kann auf `true` gesetzt werden, um beim Export Projektdaten zu berücksichtigen. |
| `ExportMetadataForInstitution` | Text | | Dieser Wert kann auf `true` gesetzt werden, um beim Export Daten zur Institution zu berücksichtigen. |
