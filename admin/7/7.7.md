...
| `jwtSecret` | Text | | Die REST-API von Goobi Workflow verwendet zur Authentifizierung ein JSON Web Token (JWT). Dieses wird hier spezifizier. Um eine authentifizierte Anfrage an die REST-API zu schicken, muss das Token in der Anfrage angegeben werden. |
