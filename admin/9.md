...
Der Konfigurations-Parser wurde so umgestellt, dass die Kommata und Backslashes nicht mehr escaped werden müssen. Es müssen alle Vorkommnisse von `\,` durch `,`  und  `\\` durch `\`ersetzt werden. Betroffen sind die folgenden Dateien unterhalb von `/opt/digiverso/goobi/config/`:
