...
Dieses GoobiScript aktualisiert die interne History-Tabelle, die Goobi über jeden Vorgang vorhält und aus der einige Statistiken generiert werden können. Es werden keine Parameter für die Ausführung dieses GoobiScripts benötigt.

Das GoobiScript `updateMetadata` sorgt dafür, dass die interne `metadata` Tabelle der Goobi-Datenbank aktualisiert wird. Dies ist dann wichtig, wenn die Metadaten außerhalb von Goobi verändert wurden oder wenn ein neues Indexfeld definiert wurde.
