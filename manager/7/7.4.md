...
Das GoobiScript `metadataChangeType` ändert den Typ eines Metadatums. Der Aufruf verlangt drei Parameter: `oldMetadata` ist der alte, zu ändernde Metadatentyp, `newMetadata` ist der neue Metadatentyp und `position` legt fest, an welchem Element das neue Metadatum angesiedelt wird. Für `position` gibt es drei verschiedene mögliche Werte, die sich je nach Strukturtyp des Vorgangs anders verhalten:

| `position`-Wert | Einbändig | Mehrbändig |
| :--- | :--- | :--- |
| `top` | Hauptelement | Anchor-Element |
| `child` | - nicht anwendbar - | Volume |
| `work` | Hauptelement | Volume |
